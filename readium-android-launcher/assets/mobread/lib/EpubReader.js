    ...
    var loadToc = function(dom){
        ...
        readium.reader.on(ReadiumSDK.Events.CONTENT_DOCUMENT_LOADED, function ($iframe, spineItem)
        {
        	if (window.jsInterface){
        		window.jsInterface.startPage(spineItem.index,spineItem.spine.items.length);
        	}
        });
        ...
        readium.reader.on(ReadiumSDK.Events.CONTENT_DOCUMENT_LOADED, function ($iframe, spineItem)
        {
            ...
            var viewportTag=$iframe.contents().find('meta[name="viewport"]');
            var viewportContent=viewportTag.attr('content');
            var viewportSizes=getViewportSize(viewportContent);
            if (window.jsInterface){
            	window.jsInterface.adjustScreen(viewportSizes.width,viewportSizes.height);
            }
        });
        
        var getViewportSize=function(content){
        	if (content==null || content==undefined || content===''){
        		return { 'width': 768, 'height': 1024 };
        	}
        	var measureParts=content.split(',');
        	var width=0,height=0;
        	if (measureParts.length==2){
        		width=measureParts[0].split('=').length==2?
        				measureParts[0].split('=')[1]:0;
				height=measureParts[1].split('=').length==2?
        				measureParts[1].split('=')[1]:0;
        	}
...
catch (e)
{
    ...
    var isUsingContinuousScrollMode=false;
    ...
    var installReaderEventHandlers = function(){
        ...
        // switch between from/to normal scroll mode to/from continuous mode
        $('#btnScrollContinuous').on('click',function(){
        	if (isUsingContinuousScrollMode){
        		readium.reader.updateSettings({ isScrollContinuous : false });
        		isUsingContinuousScrollMode=false;
        		//$('#btnScrollContinuous span').addClass('glyphicon-road');
        		//$('#btnScrollContinuous span').removeClass('glyphicon-file');
        		$('#btnScrollContinuous img').attr('src','../../images/main/ic_flow_view.png');
        	}else{
        		readium.reader.updateSettings({ isScrollContinuous : true });
        		isUsingContinuousScrollMode=true;
        		//$('#btnScrollContinuous span').addClass('glyphicon-file');
				//$('#btnScrollContinuous span').removeClass('glyphicon-road');
        		$('#btnScrollContinuous img').attr('src','../../images/main/ic_single_view.png');
        	}
        });
        
        $('#btnFullScreen').on('click',function(){
        	window.parent.epubReader.switchBookScale();
        });
        ...
        var setTocSize = function(){
            ...
            var appHeight = $(document.body).height() - $('#app-container')[0].offsetTop - bottomBarHeight;
            $('#app-container').height(appHeight);
            $('#readium-toc-body').height(appHeight);
            $('#reading-area').height(appHeight);
